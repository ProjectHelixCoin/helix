...
class Node :
	def __init__(self, app, nodeIndex) :
		...
		self._helixd = self._app.getHelixd()
		self._helixCli = self._app.getHelixCli()
		self._daemonProcess = None

	def createDataDir(self, nodeCount, masterNodePrivateKey = None) :
		makeDirs(self._nodePath)
		writeFile(
			os.path.join(self._nodePath, 'helix.conf'),
			self._generateHelixConf(nodeCount, masterNodePrivateKey)
		)
		
	def startNode(self) :
		self._daemonProcess = subprocess.Popen([ self._helixd, '-datadir=' + self._nodePath, '-daemon' ])
	...
	def executeCli(self, *args) :
		...
		output = executeCommand(self._helixCli, '-datadir=' + self._nodePath, *normalizedArgs)
	...
	def _generateHelixConf(self, nodeCount, masterNodePrivateKey) :
		result = ""
		result += "regtest=1\n"
		result += "server=1\n"
		result += "debug=1\n"
...
class Application :
	...
	def getHelixCli(self) :
		return self._helixCli
