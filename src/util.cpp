...
int nAnonymizeHelixAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "helix" is a composite category enabling all Helix-related debug output
            if (ptrCategory->count(string("helix"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Helix
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Helix
// Mac: ~/Library/Application Support/Helix
// Unix: ~/.helix
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Helix";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "helix.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty helix.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override helix.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "helixd.pid"));
