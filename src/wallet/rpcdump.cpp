...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"helixprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"helixprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (IsHex(params[0].get_str())) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Helix address or script");
    }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"helixaddress\"\n"
            "\nReveals the private key corresponding to 'helixaddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"helixaddress\"   (string, required) The helix address for the private key\n"
            "\nResult:\n"
...
UniValue dumpwallet(const UniValue& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by HELIX %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
...
UniValue dumpallprivatekeys(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "they will have FULL ACCESS to STEAL your Helix. Giving this file to someone\n"
            "is the same thing as giving them all of the Helix in your wallet! Never send\n"
            "this file to ANYONE that you do not trust with all of your Helix!!!\n"
           "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename\n"
            "\nExamples:\n" +
    ...
    // produce output
    file << strprintf("# Wallet private key dump file created by Helix %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
    file << strprintf("# * Created on %s\n", EncodeDumpTime(GetTime()));
    file << strprintf("# * Best block at time of backup was %i (%s),\n", chainActive.Height(), chainActive.Tip()->GetBlockHash().ToString());
    file << strprintf("#   mined on %s\n", EncodeDumpTime(chainActive.Tip()->GetBlockTime()));
    file << "\n";
...
UniValue bip38encrypt(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38encrypt \"helixaddress\"\n"
            "\nEncrypts a private key corresponding to 'helixaddress'.\n"
            "\nArguments:\n"
            "1. \"helixaddress\"   (string, required) The helix address for the private key (you must hold the key already)\n"
...
UniValue makeairdropfile(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            ...
            "\nSign all Helix addresses in JSON format and output to file to be used for airdrop.\n"
           "\nArguments:\n"
            "1. \"bsc_address\"    (string, required) The BSC address to receive tokens\n"
            "2. airdrop_number    (integer, required) The airdrop number\n"
            "3. \"filename\"    (string, required) The path (optional) and filename for the proof of Helix address ownership.\n"
            "                                      If path is not specified, file is created in Helix data directory.\n"
            "\nExamples:\n" +
