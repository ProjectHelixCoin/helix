                                                          ...
                                                          currentBalance(-1)
{
    ...
    ui->zHLIXpayAmount->setAttribute(Qt::WA_MacShowFocusRect, 0);


    // "Spending 999999 zHLIX ought to be enough for anybody." - Bill Gates, 2017
    ui->zHLIXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzHLIXSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zHLIXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzHLIX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zHLIX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zHLIX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zHLIX in ") +
...
void PrivacyDialog::on_pushButtonSpendzHLIX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zHLIX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zHLIX
        sendzHLIX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zHLIX
    sendzHLIX();
}

void PrivacyDialog::on_pushButtonZHlixControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ZHlixControlDialog* zHlixControl = new ZHlixControlDialog(this);
    zHlixControl->setModel(walletModel);
    zHlixControl->exec();
}

void PrivacyDialog::setZHlixControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzHLIXSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzHLIX()
{
    ...
    double dAmount = ui->zHLIXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zHLIX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " HLIX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zHLIX</b>";
    ...
    // use mints from zHLIX selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZHlixControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZHlixControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zHLIX require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zHLIX"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZHLIX_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zHLIX require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zHLIX"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zHLIX was spent successfully update the addressbook with the label
    ...
    // Clear zhlix selector in case it was used
    ZHlixControlDialog::setSelectedMints.clear();
    ui->labelzHLIXSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zHLIX Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Hlix, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zHLIX Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zhlixTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zHLIX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zHLIX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zHLIX "));
    ui->labelzHLIXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzHLIXSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzHLIX->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zHLIX
        ui->pushButtonMintzHLIX->setEnabled(false);
        ui->pushButtonMintzHLIX->setToolTip(tr("zHLIX is currently disabled due to maintenance."));

        // Spend zHLIX
        ui->pushButtonSpendzHLIX->setEnabled(false);
        ui->pushButtonSpendzHLIX->setToolTip(tr("zHLIX is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zHLIX
        ui->pushButtonMintzHLIX->setEnabled(true);
        ui->pushButtonMintzHLIX->setToolTip(tr("PrivacyDialog", "Enter an amount of HLIX to convert to zHLIX", 0));

        // Spend zHLIX
        ui->pushButtonSpendzHLIX->setEnabled(true);
        ui->pushButtonSpendzHLIX->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}